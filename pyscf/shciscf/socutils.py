...
def get_fso2e_0(mol, x, rp, pLL, pLS, pSS):
...
def get_fso2e_2(mol, x, rp, pLL, pLS, pSS):
        ...
        for j in range(0, nbas):
            for k in range(0, nbas):
                for l in range(0, nbas):
                    ...
                    lll = shl_slice[l + 1]
                    start = time.clock()
                    ddint = mol.intor('int2e_ip1ip2_sph', comp=9, shls_slice=[ii, iii, jj, jjj, kk, kkk, ll,
                                                                              lll]).reshape(3, 3, -1)
...
def get_fso2e_1c(mol, x, rp, pLL, pLS, pSS):
        ...
        for j in range(0, nbas):
            for k in range(0, nbas):
                for l in range(0, nbas):
                    ...
                    lll = shl_slice[l + 1]
                    start = time.clock()
                    h2ao = mol.intor('cint2e_p1vxp1_sph',
                                     comp=3,
                                     aosym='s1',
                                     shls_slice=[ii, iii, jj, jjj, kk, kkk, ll,
                                                 lll]).reshape(3, shl_size[i], shl_size[j], shl_size[k], shl_size[l])
                    end = time.clock()
                    print("Time elapsed for integral calculation:", end - start, i, j, k, l, nbas)
                    start = time.clock()
...
def get_hso1e_bp1c(mol, dm, atomlist):
    ...
    for iatom in atomlist:
        if shl_size[iatom] > max_basis:
            ...
            h2ao = mol.intor('cint2e_p1vxp1_sph',
                             comp=3,
                             aosym='s1',
                             shls_slice=[ibegin, iend, ibegin, iend, ibegin, iend, ibegin,
                                         iend]).reshape(3, shl_size[iatom], shl_size[iatom], shl_size[iatom],
                                                        shl_size[iatom])
            end = time.clock()
            print("Time elapsed for integral calculation:", end - start, iatom, nbas)
            start = time.clock()
...
def writeSOCIntegrals(mc,
    ...
    if (pictureChange2e == "bp"):
        hso1e += -(alpha)**2 * 0.5 * get_hso1e_bp(xmol, dm)
        ...
        pLL, pLS, pSS = get_p(dm / 2.0, x, rp)
        hso1e += -(alpha)**2 * 0.5 * get_fso2e_1c(xmol, x, rp, pLL, pLS, pSS)
    ...
    elif (pictureChange1e == "bp1c"):
        hso1e += (alpha)**2 * 0.5 * get_wso_1c(xmol, atomlist)
    elif (pictureChange1e == "x2c1"):
        wso = (alpha)**2 * 0.5 * get_wso(xmol)
        hso1e += get_hso1e(wso, x, rp)
    elif (pictureChange1e == "x2cn"):
        h1e_2c = (alpha)**2 * 0.5 * x2c.get_hcore(xmol)
...
def doSOC(mc, pictureChange1e = "bp", pictureChange2e = "bp1c", uncontract = False, atomlist = None, FCIDUMP = True):
    if (FCIDUMP):
        mch = shci.SHCISCF(mc.mol, mc.norb, mc.nelec)
        mch.fcisolver.onlywriteIntegral = False
        shci.dryrun(mch, mc.mo_coeff)
    writeGTensorIntegrals(mc, atomlist = atomlist)
    writeSOCIntegrals(mc, pictureChange1e = pictureChange1e, pictureChange2e = pictureChange2e, uncontract = uncontract, atomlist = atomlist)
